...
export default function AshaHomeScreen() {
  ...
  return <Screen scrollable header={<ScreenHeader contextLabel="ASHA Portal" title={`${getGreeting()}, ${getFirstName('Sunita Devi')}`} />}>
    ...
    <View className="mb-4 flex-row flex-wrap gap-3">
      <Stat title="Active Cases" value={String(activeCount)} icon={<ClipboardList size={19} color="#0284C7" />} onPress={() => router.push('/(asha)/cases')} />
      <Stat title="High Priority" value={String(highCount)} icon={<AlertTriangle size={19} color="#BE123C" />} onPress={() => router.push('/(asha)/cases')} />
    ...
    <View className="mb-3 flex-row items-center justify-between"><AppText variant="title">Needs Attention</AppText><Pressable onPress={() => router.push('/(asha)/cases')}><AppText variant="label" className="text-primary">View all</AppText></Pressable></View>
